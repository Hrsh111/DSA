...
from config import GROQ_API_KEY, GROQ_MODEL, OLLAMA_MODEL, OLLAMA_URL, TOPICS
...
Examples:
  ...
  python main.py --topic arrays --diff medium      Specific topic
  ...
  python main.py --local                           Force Ollama (local, slower)
    ...
    parser.add_argument("--topic", choices=TOPICS, help="Start with a specific topic")
    parser.add_argument("--diff", choices=["easy", "medium", "hard"], default="medium", help="Difficulty level")
...
def main():
    ...
    ui.print_info(f"Loaded {bank.count()} questions across {len(bank.get_topics())} topics")
    ...
    interviewer = Interviewer(llm, voice, watcher, bank)
    ...
    if args.topic:
        interviewer.topic = args.topic
        interviewer.difficulty = args.diff
        from interviewer import State
        interviewer.state = State.ASK_QUESTION
